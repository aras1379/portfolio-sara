...
const handleDragEnd = (_: MouseEvent | TouchEvent | PointerEvent, info: PanInfo): void => {
  const finalAngle = rotation.get(); // Remove velocity calculation
  rotation.set(finalAngle);
  if (autoplay) {
    startInfiniteSpin(finalAngle); // Restart at constant speed
  }
};
          ...
          {galleryImages.map((url, i) => (
            ...
            >
              <img
                src={url}
                alt="gallery"
                className="pointer-events-none h-[300px] w-[500px] rounded-[15px] border-[3px] border-white object-cover transition-transform duration-300 ease-out group-hover:scale-105 sm:h-[150px] sm:w-[320px]"
              />
            </div>
          ))}
        </motion.div>
